...
class CpropClass
{
    ...
    //выводит поля разных параметров в настройках элемента
    public static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        //перебираем параметры св-ва и в зависимости от типа св-ва
    ...
    //выводит html для настроек свойства инфоблока
    ...
    //обрабатываем настройки свойства (ниже парметров)
    ...
    //если св-во обязательное проверяет чтобы все поля не были пустыми
    ...
    //для сохранения значений полей в нужном виде (json) комплексного св-ва в БД
    public static function ConvertToDB($arProperty, $arValue)
    {
        ...
        foreach ($arValue['VALUE'] as $code => $value) {
            if ($arFields[$code]['TYPE'] === 'file') {
                $arValue['VALUE'][$code] = self::prepareFileToDB(
                    $value
                );
            }
        }
    ...
    //наоборот для приведение из json в массив
    ...
    {
        \CJSCore::Init(array("jquery"));     //подключение jq
        ?>
        <script>
            function addNewRows() {
    ...
    private static function prepareSetting($arSetting)
    {
        ...
        if (!function_exists('poly\cprop\cmp')) {
            function cmp($a, $b)
            {
                if ($a['SORT'] == $b['SORT']) {
                    return 0;
        ...
        uasort($arResult, 'poly\cprop\cmp');
